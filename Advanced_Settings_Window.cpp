...
void Advanced_Settings_Window::on_Button_OK_clicked()
{
	...
	if( ui.CH_Emulator_Type->isChecked() )
	{
		Settings.setValue( "Info/Emulator_Type", "yes" );
	}
	else
	{
		Settings.setValue( "Info/Emulator_Type", "no" );
	}
	
	if( ui.CH_Emulator_Version->isChecked() )
	{
		Settings.setValue( "Info/Emulator_Version", "yes" );
	}
	else
	{
		Settings.setValue( "Info/Emulator_Version", "no" );
	}
...
{
	QEMU_Binary_Window *QEMU_Binary_Win = new QEMU_Binary_Window( this );
	
	QEMU_Binary_Win->Set_Emulator( Emulator() );
	
	if( QEMU_Binary_Win->exec() == QDialog::Accepted )
	{
		Emulator new_emul = QEMU_Binary_Win->Get_Emulator();
		...
		for( int ix = 0; ix < Emulators.count(); ix++ )
		{
			if( Emulators[ix].Get_Type() == new_emul.Get_Type() &&
				Emulators[ix].Get_Default() == "yes" )
			{
				finded = true;
			}
		}
		
		if( finded == false ) new_emul.Set_Default( "yes" );
		
		Emulators << new_emul;
		Update_Emulators_Info();
	}
	
	delete QEMU_Binary_Win;
}
...
void Advanced_Settings_Window::on_TB_Edit_Emulator_clicked()
{
	...
	if( cur_index >= 0 && cur_index < Emulators.count() )
	{
		...
		QEMU_Binary_Win->Set_Emulator( Emulators[cur_index] );
		
		if( QEMU_Binary_Win->exec() == QDialog::Accepted )
		{
			Emulators[ cur_index ] = QEMU_Binary_Win->Get_Emulator();
			Update_Emulators_Info();
		}
...
void Advanced_Settings_Window::on_TB_Use_Default_clicked()
{
	...
	if( cur_index >= 0 && cur_index < Emulators.count() )
	{
		for( int ix = 0; ix < Emulators.count(); ix++ )
		{
			if( ix == cur_index )
			{
				Emulators[ ix ].Set_Default( "yes" );
			}
			else if( Emulators[cur_index].Get_Type() == Emulators[ix].Get_Type() )
			{
				Emulators[ ix ].Set_Default( "no" );
			}
		}
...
bool Advanced_Settings_Window::Load_Emulators_Info()
{
	...
	if( emul_count <= 0 )
	{
		AQGraphic_Warning( tr("Warning!"), tr("Emulators Not Found! Please Add Emulators.") );
		return false;
	}
...
bool Advanced_Settings_Window::Save_Emulators_Info()
{
	...
	{
		AQGraphic_Warning( tr("Warning!"), tr("No Emulators Installed! Plaease Add One or Mode Emulators!") );
		return false;
	}
	...
	for( int ix = 0; ix < Emulators.count(); ++ix )
	{
		...
		if( Emulators[ix].Get_Type() == "QEMU" )
		{
			...
			if( Emulators[ix].Get_Default() == "yes" ) default_qemu = true;
		}
		else
		{
			installed_kvm = true;
			
			if( Emulators[ix].Get_Default() == "yes" ) default_kvm = true;
		}
	}
	
	Settings.setValue( "Emulators_Count", QString::number(Emulators.count()) );
...
void Advanced_Settings_Window::Update_Emulators_Info()
{
	...
	for( int ix = 0; ix < Emulators.count(); ++ix )
	{
		...
		if( Emulators[ix].Get_Type() == "QEMU" )
		{
			...
			newItem = new QTableWidgetItem( Emulators[ix].Get_QEMU_Version() );
			ui.Emulators_Table->setItem( ui.Emulators_Table->rowCount()-1, 2, newItem );
		}
		else
		{
			newItem = new QTableWidgetItem( "KVM" );
			ui.Emulators_Table->setItem( ui.Emulators_Table->rowCount()-1, 1, newItem );
			
			newItem = new QTableWidgetItem( Emulators[ix].Get_KVM_Version() );
			ui.Emulators_Table->setItem( ui.Emulators_Table->rowCount()-1, 2, newItem );
		}
		...
		newItem = new QTableWidgetItem( Emulators[ix].Get_Default() );
		ui.Emulators_Table->setItem( ui.Emulators_Table->rowCount()-1, 4, newItem );
	}
}
